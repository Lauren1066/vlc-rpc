...
async function update() {
  diff(async (status, shouldUpdate, changedFiles, changedToPlay) => {
    if (shouldUpdate) {
      const newStatus = await format(status, changedFiles, changedToPlay);

      if (status.state === 'playing') {
        activityCache = newStatus;
      }
